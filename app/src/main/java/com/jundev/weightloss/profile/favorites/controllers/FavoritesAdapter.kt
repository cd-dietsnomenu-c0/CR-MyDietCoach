    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when(getItemViewType(position)){
            ITEM_TYPE ->(holder as InteractiveVH).bind(allDiets[getRealPosition(position)].title,
                    allDiets[getRealPosition(position)].mainImage, allDiets[getRealPosition(position)].isNew, allDiets[getRealPosition(position)].shortIntroduction, allDiets[getRealPosition(position)].days.size)
